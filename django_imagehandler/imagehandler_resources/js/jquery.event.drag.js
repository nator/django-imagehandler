...
Modified by Ludwig Pettersson (http://luddep.se/) to include drag diff 
...
function handler ( event ){ 
	...
	if ( data.elem ){ 
		...
		event.diffX = data.start.X - event.offsetX; // move diff
		event.diffY = data.start.Y - event.offsetY; // move diff
		}
	...
	switch ( event.type ){
		...
		case 'mousedown':
            data.start.Y = $(elem).offset().top
            data.start.X = $(elem).offset().left
